...
const OperationCameraGetBatchCamera = "/api.camera.v1.Camera/GetBatchCamera"
...
func RegisterCameraHTTPServer(s *http.Server, srv CameraHTTPServer) {
	...
	r.GET("/api/v1/camera/batch/get", _Camera_GetBatchCamera0_HTTP_Handler(srv))
}
...
func _Camera_GetBatchCamera0_HTTP_Handler(srv CameraHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetBatchCameraRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationCameraGetBatchCamera)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetBatchCamera(ctx, req.(*GetBatchCameraRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetBatchCameraReply)
		return ctx.Result(200, reply)
	}
}
...
type CameraHTTPClient interface {
	...
	GetBatchCamera(ctx context.Context, req *GetBatchCameraRequest, opts ...http.CallOption) (rsp *GetBatchCameraReply, err error)
...
func (c *CameraHTTPClientImpl) GetBatchCamera(ctx context.Context, in *GetBatchCameraRequest, opts ...http.CallOption) (*GetBatchCameraReply, error) {
	var out GetBatchCameraReply
	pattern := "/api/v1/camera/batch/get"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationCameraGetBatchCamera))
	opts = append(opts, http.PathTemplate(pattern))
